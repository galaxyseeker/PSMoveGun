    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # 使用 API 函数获取 orientation
                orient_result = psmove_dll.PSM_GetControllerOrientation(self.controller_id, ctypes.byref(orientation))
                if orient_result != 0:
                    print(f"Failed to get orientation. Error code: {orient_result}")
                    continue

                # 使用 API 函数获取 position
#                pos_result = psmove_dll.PSM_GetControllerPosition(self.controller_id, ctypes.byref(position))
 #               if pos_result != 0:
  #                  print(f"Failed to get position. Error code: {pos_result}")
   #                 continue
