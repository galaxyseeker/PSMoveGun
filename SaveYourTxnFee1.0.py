...
class ImageButton(tk.Canvas):
    def __init__(self, parent, image_path, width, height, command=None):
        # 增加高度以容纳悬停和点击效果
        super().__init__(parent, width=width, height=height+10, highlightthickness=0, bg=bg_color)
...
def calculate_fee(event=None):
    try:
        ...
        result_amount_label.config(text=f"${total_fee:.2f}", font=("Arial", 14, "bold"), foreground="red")
        ...
        percentage_amount_label.config(text=f"{fee_percentage:.4f}%", foreground="red")
        
        # 显示交易总额
        total_value_label.config(text=f"交易总额: ")
        total_value_amount_label.config(text=f"${total_value:.2f}", foreground="red")
...
style.configure('TCheckbutton', background=bg_color, font=('Arial', 12))
...
result_amount_label = ttk.Label(result_frame, text="", font=("Arial", 14, "bold"), foreground="red")
...
percentage_amount_label = ttk.Label(percentage_frame, text="", foreground="red")
...
total_value_amount_label = ttk.Label(total_value_frame, text="", foreground="red")
...
# 添加版权信息
copyright_label = ttk.Label(root, text="© 2024 Alex Xu 版权所有", font=("Arial", 8))
